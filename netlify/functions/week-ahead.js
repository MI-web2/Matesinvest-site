...
exports.handler = async function () {
  ...
  // ---------------------------------
  // 3) Charts (ETF monthly overlay + Macro annual overlay)
  // ---------------------------------
  const disableCharts =
...
options: {
  ...
  async function fetchMacroIndicatorAUS(indicator) {
    ...
    if (!res.ok) {
      const txt = await res.text().catch(() => "");
      throw new Error(`macro-indicator ${indicator} failed: ${res.status} ${txt}`);
    }
...
async function buildMacroAnnualOverlayChart() {
  ...
  const finalEtfLabels =
    etfLabels.length === 3 ? etfLabels : defaultEtfLabels;

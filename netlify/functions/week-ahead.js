...
exports.handler = async function () {
  ...
  // ---------------------------------
  // 1) Macro bullets (local txt next to function)
  // ---------------------------------
  ...
  function formatMacroBullet(ymd, title) {
    ...
    const dow = dt.toLocaleDateString("en-AU", { weekday: "short", timeZone: "Australia/Brisbane" });
    const dayMonth = dt.toLocaleDateString("en-AU", { day: "numeric", month: "short", timeZone: "Australia/Brisbane" });
  ...
  function pickMacroBullets(items, weekStartYmd, weekEndYmd, maxBullets = 6) {
    ...
    inWeek.sort((a, b) => macroPriorityScore(b.title) - macroPriorityScore(a.title) || a.ymd.localeCompare(b.ymd));
    return inWeek.slice(0, maxBullets).map((it) => formatMacroBullet(it.ymd, it.title));
  }

  function readMacroTxtFile() {
    // 1) Preferred: same folder as this function
    const p1 = path.join(__dirname, "au-macro-key-dates-2026.txt");
    // 2) Fallback: if you ever move to a subfolder structure
    const p2 = path.join(__dirname, "..", "au-macro-key-dates-2026.txt");
    ...
    return { path: p1, text: null };
  }

  let macro = { title: "Important AU macro this week", bullets: [] };
  try {
    // Debug (remove later): confirm what files are bundled
    console.log("Functions dir files:", fs.readdirSync(__dirname));

    const { path: usedPath, text } = readMacroTxtFile();
    if (!text) throw new Error(`Macro file not found at ${usedPath}. Ensure it is committed to netlify/functions/`);
    const items = parseMacroFile(text);
  ...
  const chartSector = String(process.env.WEEK_AHEAD_CHART_SECTOR || "OZR.AU").trim();
  ...
  function rebaseTo100(values) {
    ...
    return values.map((v) => (typeof v === "number" && Number.isFinite(v) ? (v / first) * 100 : null));
  }
  ...
  function buildQuickChartUrl({ title, labels, a, b, labelA, labelB }) {
    ...
    return `https://quickchart.io/chart?width=640&height=320&c=${encodeURIComponent(JSON.stringify(cfg))}`;
  }

  function normalizeMacroSeries(raw) {
    // Accept: array, {data:[]}, {values:[]}, or object keyed by date
    if (Array.isArray(raw)) return raw;
    if (raw && Array.isArray(raw.data)) return raw.data;
    if (raw && Array.isArray(raw.values)) return raw.values;

    if (raw && typeof raw === "object") {
      // object keyed by date -> [{date, value}]
      const out = [];
      for (const k of Object.keys(raw)) {
        out.push({ date: k, value: raw[k] });
      }
      return out;
    }
    return [];
  ...
  async function buildCpiVsSectorChartUrl(sectorTicker) {
    ...
    const cpiRaw = await cpiRes.json().catch(() => null);
    const cpiRows = normalizeMacroSeries(cpiRaw);

    // Debug (remove later)
    console.log("CPI shape keys:", cpiRaw && typeof cpiRaw === "object" ? Object.keys(cpiRaw).slice(0, 10) : typeof cpiRaw);
    console.log("CPI sample:", JSON.stringify(cpiRows.slice(0, 3)));
    ...
    const secUrl =
      ...
      `?from=${isoDate(from5)}&to=${isoDate(to)}&fmt=json&api_token=${EODHD_API_TOKEN}`;
    ...
    return buildQuickChartUrl({
      title: `AU CPI vs ${sectorTicker} (rebased, ~5y)`,
      labels: months,
      a: cpiRe,
      b: secRe,

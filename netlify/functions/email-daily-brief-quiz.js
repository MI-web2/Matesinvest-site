// netlify/functions/email-daily-brief-quiz.js
// Scheduled "kicker" function: triggers the long-running background sender for the quiz version.
// This is the function you schedule in netlify.toml (Tue/Thu AEST).
...
exports.handler = async function () {
  ...
  const kickKey = `email:kick:daily-quiz:${yyyy}-${mm}-${dd}`;

  try {
    console.log(`Checking Redis for kick key: ${kickKey}`);
    const alreadyKicked = await redisGet(kickKey);
    if (alreadyKicked) {
      console.log("Daily quiz kicker already ran for", `${yyyy}-${mm}-${dd}`);
    ...
    // Ensure siteUrl starts with http:// or https://
    const normalizedUrl = siteUrl.startsWith('http') ? siteUrl : `https://${siteUrl}`;
    const bgUrl = `${normalizedUrl}/.netlify/functions/email-daily-brief-quiz-background`;
    ...
    console.log("Triggered background daily quiz:", resp.status, txt);

    // Return 200 regardless to avoid retries spamming
    return { statusCode: 200, body: `Kicked background: ${resp.status}` };
  } catch (err) {
    console.error("email-daily-brief-quiz kicker error", err.name, err.message);
    if (err.stack) {

...
// Uses the EODHD bulk endpoint instead of hitting /eod for each symbol.
//
...
// Same normaliser we’ve used elsewhere (BHP.AX -> BHP)
...
function readUniverseSync() {
  ...
  throw new Error(
    "asx-universe.txt not found next to the function (or in netlify/functions)."
  );
}
...
exports.handler = async function () {
  ...
  for (const r of json) {
    ...
    const prevClose = Number(r.previousClose || r.previous_close || NaN);

    let pctChange = null;
    if (Number.isFinite(close) && Number.isFinite(prevClose) && prevClose !== 0) {
      pctChange = Number(((close - prevClose) / prevClose) * 100);
    }
